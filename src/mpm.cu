...
Simulation::Simulation(const CLIOptions &opts,
  ...
  N2 = par.N * par.N;
  host_grid = new Vec4[par.N * par.N * par.N];
...
void Simulation::resetGrid() {
  ...
  checkGpuError(cudaGetLastError());
  checkGpuError(cudaDeviceSynchronize()); // for debugging. can be removed.
}

void Simulation::particleToGridTransfer(std::vector<SimObject> & objects) {
  particlesToDevice();
  int n_threads = 64;
  int grid_width = active_particle_count / n_threads;
  ...
  //      if (cell[3] != 0.0)
  //        std::cout << "cell " << i * N * N + j *N +k << " " << cell.transpose() << std::endl;
  //    }
  //  }
  //}
}
...
void Simulation::gridToParticleTransfer(std::vector<SimObject> & objects) {
  // Grid-to-particle.
  for(SimObject & object : objects) { // loop through objects
...
Vec4& Simulation::getCell(int i, int j, int k) {
  return host_grid[N2 * i + N * j + k];
}

void Simulation::gridHostToDevice() {
  cudaMemcpy(device_grid, host_grid, sizeof_grid, cudaMemcpyHostToDevice);
}

void Simulation::gridDeviceToHost() {
